...
public class DiversityComputingTest {


    private AppInterface testApp;
    private DiversityComputing testDiv;
    ...
    public void setUp() {
        ...
        testDiv = new DiversityComputing(testApp);
    ...
    public void testComputeAllSitesResults() {
        ...
        assertEquals(testDiv.computeAllSitesResults(testSites).size(), 0);

        testSites.add(s1);
        assertEquals(testDiv.computeAllSitesResults(testSites).size(), 1);

        testSites.add(s2);
        assertEquals(testDiv.computeAllSitesResults(testSites).size(), 2);
    }
}
