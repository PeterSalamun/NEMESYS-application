...
public class DominancyComputingTest {


    private AppInterface testApp;
    private DominancyComputing testDom;
    ...
    public void setUp() {
        ...
        testDom = new DominancyComputing(testApp);
    ...
    public void testComputeAllSitesResults() {
        ...
        assertEquals(testDom.computeAllSitesResults(testSites).size(), 0);

        testSites.add(s1);
        assertEquals(testDom.computeAllSitesResults(testSites).size(), 1);

        testSites.add(s2);
        assertEquals(testDom.computeAllSitesResults(testSites).size(), 2);
    }
}
