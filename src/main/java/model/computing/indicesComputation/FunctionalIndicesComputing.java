...
public class FunctionalIndicesComputing extends GeneralComputing {
    ...
    //REQUIRES: String
    //EFFECTS: return boolean for String guild is not equals "H" and 0 < guild < 6
    private boolean checkNotHerbivoryGuild(String guild) {
        ...
        return (!sub1.equals("H") && (sub2 > 0 && sub2 < 6));
    }

    //REQUIRES: String, double, String
    //EFFECTS: return double (double genusAbu* factor) or 0 if no condition is fulfill
    ...
    //EFFECTS: methods bellow computes and return double indices values based on inputs
    ...
    protected double getSiteAbundance(ArrayList<Sample> samplesList) {
        ...
        for (Sample s : samplesList)
            if (database.get(s.getTaxaName()).getCp() > 0)
